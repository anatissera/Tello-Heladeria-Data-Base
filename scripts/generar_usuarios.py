...
# IDs de sucursales que ya cargaste
SUCURSALES = [
    (1, "Microcentro"),
    (2, "Catamarca"),
    (3, "Barrio Norte"),
...
NUM_EMPLEADOS_ACTIVOS = 8   # empleados activos por sucursal
NUM_ENCARGADOS_ACTIVOS = 3  # encargados activos por sucursal
...
# Generar empleados por sucursal
for id_suc, nombre_suc in SUCURSALES:
    # Primero los encargados activos
    for _ in range(NUM_ENCARGADOS_ACTIVOS):
        ...
        empleados.append([dni, True])  # es_encargado = True
    
    # Luego los demás empleados activos
    ...
    # Agregar algunos empleados inactivos extra
...
# Administradores (todos en sucursal 7)
for _ in range(4):  # 4 activos
    dni = str(fake.unique.random_number(digits=8))
...
# Al menos 1 admin inactivo
dni = str(fake.unique.random_number(digits=8))
usuarios.append([dni, fake.name(), fake.date_of_birth(minimum_age=25, maximum_age=65),
                 fake.unique.email(), 7, False, "administrador", False])
admins.append([dni])

# Proveedores (todos en sucursal 7)
for _ in range(2):  # 2 activos
    dni = str(fake.unique.random_number(digits=8))
    usuarios.append([dni, fake.name(), fake.date_of_birth(minimum_age=18, maximum_age=65),
                     fake.unique.email(), 7, True, "proveedor", False])
    proveedores.append([dni])

# Al menos 1 proveedor inactivo
dni = str(fake.unique.random_number(digits=8))
